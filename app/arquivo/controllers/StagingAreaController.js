			...
			.then(async (worksheet) => {
				...
				worksheet.eachRow({ includeEmpty: false }, async function(row) {
					...
					if(flag){
						...
						rows.push({data, dia_semana, hora, endereco, bairro, municipio, cod_tipo, descricao, cod_sub_tipo, desc_sub_tipo, sit_encontrada, descricao_finalizacao, historico, estado});
					}
					flag = true;
				});
